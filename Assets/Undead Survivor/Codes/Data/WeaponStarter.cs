...
public class WeaponStarter
{
    ...
    {
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Sword_The_Flute);
    ...
    void Sword_The_Black() //일반공격 피해가 {0:P}증가함. 일반공격 사용 시 공격력의 {1:P}에 해당하는 HP를 회복함. 재사용 대기시간 10초
    {
        ...
        SkillData.ParameterWithKey baseAttack = skillData.Get(characterParam.skillBasic);
    ...
    void Sword_Aquila_Favonia() //공격력이 {0:P}증가함. 적에게 체력이 감소된 경우 공격력의 {1:P}만큼 HP를 회복하고 그 두배만큼의 피해를 주변 적에게 줌. 재사용 대기시간 15초
    {
        ...
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Sword_Aquila_Favonia);
    ...
    void Sword_Skyword_Blade() //행운이 {0} 증가함. 장착 캐릭터의 원소폭발 발동 시 이동속도와 10% 증가 일반공격 쿨타임 10% 감소. 일반공격 대미지 {1:P} 증가. 지속 12초
    {
        ...
        SkillData.ParameterWithKey burst = skillData.Get(skillData.Get(characterParam.skill).burst);
        ...
        SkillData.ParameterWithKey baseAttack = skillData.Get(characterMain.skillBasic);
    ...
    {
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Claymore_The_Bell);
    ...
    {
        statBuff.allDamageAdd(weaponParameter.valueSums[0]);
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Claymore_Skyward_Pride);
    ...
    void Spear_Skyward_Spine() //행운 {0} 증가 일반공격 재사용 대기시간 12% 감소. 2초마다 가장 가까운 적 방향으로 진공 칼날을 날려 {1:P}의 피해를 줌
    {
        ...
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Spear_Skyward_Spine);
        skillParameter.parameter.damage = weaponParameter.valueSums[1];
        skillParameter.AddStartListener(() =>
        {
            WeaponFade();
    ...
    {
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Catalist_Eye_Of_Perception);
    ...
    void Catalist_Skyward_Atlas() //원소 피해 보너스 {0:P} 증가. 30초마다 가장 가까운 적을 향해 발사해 {1:P}의 피해를 주는 투사체를 지속적으로 생성함. 지속 15초
    {
        ...
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Catalist_Skyward_Atlas);
        skillParameter.parameter.damage = weaponParameter.valueSums[1];
        skillParameter.AddStartListener(() =>
        {
            WeaponFade(true);
    ...
    {
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Bow_Viridescent_Hunt);
        skillParameter.parameter.coolTime = weaponParameter.valueSums[0];
        skillParameter.parameter.damage = weaponParameter.valueSums[1];
        skillParameter.AddStartListener(() =>
        {
    ...
    {
        statBuff.luck += weaponParameter.valueSums[0];
        SkillData.ParameterWithKey skillParameter = skillData.Get(SkillName.Weapon_Bow_SkyWard_Harp);
